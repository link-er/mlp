...
public class MultiGas {
	...
	private double[] getRandomPoint(int area){
		...
		switch(area){
			case 0: randomPoint = drawRandomPoint(0.0, 0.3);
					break;
			case 1: randomPoint = drawRandomPoint(0.35, 0.65);
					break;
			case 2: randomPoint = drawRandomPoint(0.7, 1.0);
					break;
		}
	return randomPoint;
	}
	...
	private double[] drawRandomPoint(double minValue, double maxValue){
		...
		Random rand = new Random();
		for(int i=0; i<N; i++){
			double randCoordinate = minValue + (maxValue - minValue) * rand.nextDouble();
...
//		TODO find nearest center by euclidianMetrics and return index of Gas and center; call SortCenters
	...
	public void writePatternsToFile(){
		try{
			PrintWriter fout = new PrintWriter(new BufferedWriter(new FileWriter("patterns.dat")));
  			for (int i = 0; i < tMax; i++) {
    			for(int j=0; j<N; j++){
...
//		number of partners
		int M = 5;
		int[] structure = {2, 2, 3, 3, 7}; //the length of this array should be always equal M
//		input dimension
		int N = 2;
